...
with tab1:
    ...
    with r1_col1:
        ...
        state_sales = filtered_df.groupby("State")["Sales_Revenue"].sum()
        st.bar_chart(state_sales)
        
    with r1_col2:
        st.markdown("**Vehicle Model Demand Breakdown**")
        model_sales = filtered_df.groupby("Car_Model")["Sales_Revenue"].sum()
...
with tab2:
    ...
    with sc1:
        ...
        dealer_service = filtered_df.groupby("Dealership")["Service_Revenue"].sum()
        st.bar_chart(dealer_service)
        
    with sc2:
        st.markdown("**Service Type Revenue Distribution**")
        serv_type_df = filtered_df.groupby("Service_Type")["Service_Revenue"].sum()
        st.bar_chart(serv_type_df)
